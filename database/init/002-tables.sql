...
CREATE TABLE IF NOT EXISTS phone_prefixes (
    ...
    CONSTRAINT fk_phone_prefixes_country
        FOREIGN KEY (country_id)
        REFERENCES countries(id),

    CONSTRAINT chk_phone_prefixes_dial_code
        CHECK (dial_code ~ '^[+][0-9]{1,6}$'),

    CONSTRAINT uq_phone_prefixes_country_dial_code
        UNIQUE (country_id, dial_code)
);
...
CREATE TABLE IF NOT EXISTS languages (
    ...
    CONSTRAINT chk_languages_code_format
        CHECK (code ~ '^[a-z]{2}(-[A-Z]{2})?$'),

    CONSTRAINT chk_languages_name_length
        CHECK (length(trim(name)) >= 2),

    CONSTRAINT chk_languages_native_name_length
        CHECK (length(trim(native_name)) >= 2)
);
...
CREATE TABLE IF NOT EXISTS business_types (
    ...
    CONSTRAINT chk_business_types_code_format
        CHECK (code ~ '^[a-z0-9_]+$'),

    CONSTRAINT chk_business_types_name_length
        CHECK (length(trim(name)) >= 2)
);
...
CREATE TABLE IF NOT EXISTS payment_providers (
    ...
    CONSTRAINT chk_payment_providers_code_format
        CHECK (code ~ '^[a-z0-9_]+$'),

    CONSTRAINT chk_payment_providers_name_length
        CHECK (length(trim(name)) >= 2)
);
...
CREATE TABLE IF NOT EXISTS availability_override_types (
    ...
    CONSTRAINT chk_availability_override_types_code_format
        CHECK (code ~ '^[a-z0-9_]+$'),

    CONSTRAINT chk_availability_override_types_name_length
        CHECK (length(trim(name)) >= 2)
);
...
CREATE TABLE IF NOT EXISTS notification_types (
    ...
    CONSTRAINT chk_notification_types_code_format
        CHECK (code ~ '^[a-z0-9_]+$'),

    CONSTRAINT chk_notification_types_name_length
        CHECK (length(trim(name)) >= 2)
);
...
CREATE TABLE IF NOT EXISTS users (
    ...
    CONSTRAINT fk_users_phone_prefix
        FOREIGN KEY (phone_prefix_id)
        REFERENCES phone_prefixes(id),
    ...
    CONSTRAINT chk_users_phone_number_format
        CHECK (phone_number IS NULL OR phone_number ~ '^[0-9]{6,20}$'),

    CONSTRAINT chk_users_phone_e164_format
        CHECK (phone_e164 IS NULL OR phone_e164 ~ '^[+][0-9]{7,20}$'),

    CONSTRAINT chk_users_avatar_url_format
...
CREATE TABLE IF NOT EXISTS businesses (
    ...
    CONSTRAINT fk_businesses_country
        FOREIGN KEY (country_id)
        REFERENCES countries(id),

    CONSTRAINT fk_businesses_currency
        FOREIGN KEY (currency_code)
        REFERENCES currencies(code),

    CONSTRAINT fk_businesses_language
        FOREIGN KEY (default_language_code)
        REFERENCES languages(code),

    CONSTRAINT fk_businesses_phone_prefix
        FOREIGN KEY (phone_prefix_id)
        REFERENCES phone_prefixes(id),

    CONSTRAINT uq_businesses_id_business_id
        UNIQUE (id),

    CONSTRAINT chk_businesses_name_length
        CHECK (length(trim(name)) >= 2),

    CONSTRAINT chk_businesses_slug_format
        CHECK (slug ~ '^[a-z0-9]+(?:-[a-z0-9]+)*$'),

    CONSTRAINT chk_businesses_timezone_length
        CHECK (length(trim(timezone)) >= 3),

    CONSTRAINT chk_businesses_logo_url_format
        CHECK (logo_url IS NULL OR logo_url ~* '^https?://'),

    CONSTRAINT chk_businesses_google_maps_url_format
        CHECK (google_maps_url IS NULL OR google_maps_url ~* '^https?://'),

    CONSTRAINT chk_businesses_waze_url_format
        CHECK (waze_url IS NULL OR waze_url ~* '^https?://'),

    CONSTRAINT chk_businesses_latitude
        CHECK (latitude IS NULL OR latitude BETWEEN -90 AND 90),

    CONSTRAINT chk_businesses_longitude
        CHECK (longitude IS NULL OR longitude BETWEEN -180 AND 180),

    CONSTRAINT chk_businesses_phone_number_format
        CHECK (phone_number IS NULL OR phone_number ~ '^[0-9]{6,20}$'),

    CONSTRAINT chk_businesses_phone_e164_format
        CHECK (phone_e164 IS NULL OR phone_e164 ~ '^[+][0-9]{7,20}$')
);
...
CREATE TABLE IF NOT EXISTS branches (
    ...
    CONSTRAINT uq_branches_id_business_id
    ...
    CONSTRAINT fk_branches_phone_prefix
        FOREIGN KEY (phone_prefix_id)
        REFERENCES phone_prefixes(id),

    CONSTRAINT chk_branches_name_length
        CHECK (length(trim(name)) >= 2),

    CONSTRAINT chk_branches_google_maps_url_format
        CHECK (google_maps_url IS NULL OR google_maps_url ~* '^https?://'),

    CONSTRAINT chk_branches_waze_url_format
        CHECK (waze_url IS NULL OR waze_url ~* '^https?://'),

    CONSTRAINT chk_branches_latitude
        CHECK (latitude IS NULL OR latitude BETWEEN -90 AND 90),

    CONSTRAINT chk_branches_longitude
        CHECK (longitude IS NULL OR longitude BETWEEN -180 AND 180),

    CONSTRAINT chk_branches_phone_number_format
        CHECK (phone_number IS NULL OR phone_number ~ '^[0-9]{6,20}$'),

    CONSTRAINT chk_branches_phone_e164_format
        CHECK (phone_e164 IS NULL OR phone_e164 ~ '^[+][0-9]{7,20}$')
);

CREATE UNIQUE INDEX IF NOT EXISTS branches_business_name_unique_idx
...
CREATE TABLE IF NOT EXISTS roles (
    ...
    CONSTRAINT uq_roles_business_code
        UNIQUE (business_id, code),

    CONSTRAINT chk_roles_code_format
        CHECK (code ~ '^[a-z0-9_]+$'),

    CONSTRAINT chk_roles_name_length
        CHECK (length(trim(name)) >= 2)
);
...
CREATE TABLE IF NOT EXISTS permissions (
    ...
    CONSTRAINT chk_permissions_description_length
...
CREATE TABLE IF NOT EXISTS business_invitations (
    ...
    CONSTRAINT fk_business_invitations_role
        FOREIGN KEY (role_id)
        REFERENCES roles(id),

    CONSTRAINT fk_business_invitations_invited_by_user
        FOREIGN KEY (invited_by_user_id)
        REFERENCES users(id),

    CONSTRAINT chk_business_invitations_email_format
        CHECK (email ~* '^[A-Z0-9._%+-]+@[A-Z0-9.-]+[.][A-Z]{2,}$'),

    CONSTRAINT chk_business_invitations_token_hash_length
        CHECK (length(token_hash) >= 20),

    CONSTRAINT chk_business_invitations_expires_at
...
CREATE TABLE IF NOT EXISTS business_invite_links (
    ...
    CONSTRAINT fk_business_invite_links_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_business_invite_links_role
        FOREIGN KEY (role_id)
        REFERENCES roles(id),

    CONSTRAINT fk_business_invite_links_created_by_user
        FOREIGN KEY (created_by_user_id)
        REFERENCES users(id),

    CONSTRAINT chk_business_invite_links_token_hash_length
        CHECK (length(token_hash) >= 20),

    CONSTRAINT chk_business_invite_links_max_uses
        CHECK (max_uses IS NULL OR max_uses > 0),

    CONSTRAINT chk_business_invite_links_uses_count
        CHECK (uses_count >= 0),

    CONSTRAINT chk_business_invite_links_uses_not_above_max
...
CREATE TABLE IF NOT EXISTS workers (
    ...
    CONSTRAINT uq_workers_id_business_id
        UNIQUE (id, business_id),
    ...
    CONSTRAINT fk_workers_phone_prefix
        FOREIGN KEY (phone_prefix_id)
        REFERENCES phone_prefixes(id),

    CONSTRAINT chk_workers_first_name_length
        CHECK (length(trim(first_name)) >= 2),

    CONSTRAINT chk_workers_email_format
        CHECK (email IS NULL OR email ~* '^[A-Z0-9._%+-]+@[A-Z0-9.-]+[.][A-Z]{2,}$'),

    CONSTRAINT chk_workers_phone_number_format
        CHECK (phone_number IS NULL OR phone_number ~ '^[0-9]{6,20}$'),

    CONSTRAINT chk_workers_phone_e164_format
        CHECK (phone_e164 IS NULL OR phone_e164 ~ '^[+][0-9]{7,20}$'),

    CONSTRAINT chk_workers_color_format
...
CREATE TABLE IF NOT EXISTS worker_aliases (
    ...
    CONSTRAINT fk_worker_aliases_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_worker_aliases_worker_business
        FOREIGN KEY (worker_id, business_id)
        REFERENCES workers(id, business_id),

    CONSTRAINT chk_worker_aliases_alias_length
        CHECK (length(trim(alias)) >= 2),

    CONSTRAINT chk_worker_aliases_normalized_alias_length
        CHECK (length(trim(normalized_alias)) >= 2)
);

CREATE UNIQUE INDEX IF NOT EXISTS worker_aliases_worker_normalized_unique_idx
ON worker_aliases (worker_id, normalized_alias);

CREATE INDEX IF NOT EXISTS worker_aliases_business_normalized_idx
ON worker_aliases (business_id, normalized_alias);
...
CREATE TABLE IF NOT EXISTS services (
    ...
    CONSTRAINT uq_services_id_business_id
        UNIQUE (id, business_id),
    ...
    CONSTRAINT chk_services_name_length
        CHECK (length(trim(name)) >= 2),

    CONSTRAINT chk_services_base_price
        CHECK (base_price IS NULL OR base_price >= 0),

    CONSTRAINT chk_services_duration_minutes
        CHECK (duration_minutes > 0 AND duration_minutes <= 1440),

    CONSTRAINT chk_services_buffer_before
        CHECK (buffer_before_minutes >= 0),

    CONSTRAINT chk_services_buffer_after
...
CREATE TABLE IF NOT EXISTS service_aliases (
    ...
    CONSTRAINT fk_service_aliases_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_service_aliases_service_business
        FOREIGN KEY (service_id, business_id)
        REFERENCES services(id, business_id),

    CONSTRAINT chk_service_aliases_alias_length
        CHECK (length(trim(alias)) >= 2),

    CONSTRAINT chk_service_aliases_normalized_alias_length
        CHECK (length(trim(normalized_alias)) >= 2)
);

CREATE UNIQUE INDEX IF NOT EXISTS service_aliases_service_normalized_unique_idx
ON service_aliases (service_id, normalized_alias);

CREATE INDEX IF NOT EXISTS service_aliases_business_normalized_idx
ON service_aliases (business_id, normalized_alias);
...
CREATE TABLE IF NOT EXISTS worker_schedules (
    ...
    CONSTRAINT fk_worker_schedules_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_worker_schedules_worker_business
        FOREIGN KEY (worker_id, business_id)
        REFERENCES workers(id, business_id),

    CONSTRAINT fk_worker_schedules_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT chk_worker_schedules_day_of_week
        CHECK (day_of_week BETWEEN 1 AND 7),

    CONSTRAINT chk_worker_schedules_time_range
...
CREATE TABLE IF NOT EXISTS worker_availability_overrides (
    ...
    CONSTRAINT fk_worker_availability_overrides_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_worker_availability_overrides_worker_business
        FOREIGN KEY (worker_id, business_id)
        REFERENCES workers(id, business_id),

    CONSTRAINT fk_worker_availability_overrides_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT fk_worker_availability_overrides_type
        FOREIGN KEY (override_type_id)
        REFERENCES availability_override_types(id),

    CONSTRAINT fk_worker_availability_overrides_created_by_user
        FOREIGN KEY (created_by_user_id)
        REFERENCES users(id),

    CONSTRAINT chk_worker_availability_overrides_time_range
...
CREATE TABLE IF NOT EXISTS clients (
    ...
    CONSTRAINT uq_clients_id_business_id
        UNIQUE (id, business_id),
    ...
    CONSTRAINT fk_clients_default_branch_business
        FOREIGN KEY (default_branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT fk_clients_phone_prefix
        FOREIGN KEY (phone_prefix_id)
        REFERENCES phone_prefixes(id),

    CONSTRAINT chk_clients_name_length
        CHECK (name IS NULL OR length(trim(name)) >= 2),

    CONSTRAINT chk_clients_phone_number_format
        CHECK (phone_number IS NULL OR phone_number ~ '^[0-9]{6,20}$'),

    CONSTRAINT chk_clients_phone_e164_format
        CHECK (phone_e164 IS NULL OR phone_e164 ~ '^[+][0-9]{7,20}$'),
...
CREATE TABLE IF NOT EXISTS conversations (
    ...
    CONSTRAINT uq_conversations_id_business_id
        UNIQUE (id, business_id),
    ...
    CONSTRAINT fk_conversations_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT fk_conversations_client_business
        FOREIGN KEY (client_id, business_id)
        REFERENCES clients(id, business_id),

    CONSTRAINT chk_conversations_state_object
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    CONSTRAINT fk_messages_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT fk_messages_conversation_business
        FOREIGN KEY (conversation_id, business_id)
        REFERENCES conversations(id, business_id),

    CONSTRAINT fk_messages_client_business
        FOREIGN KEY (client_id, business_id)
        REFERENCES clients(id, business_id),

    CONSTRAINT chk_messages_metadata_object
        CHECK (metadata IS NULL OR jsonb_typeof(metadata) = 'object')
);

CREATE UNIQUE INDEX IF NOT EXISTS messages_external_unique_idx
...
CREATE TABLE IF NOT EXISTS message_attachments (
    ...
    CONSTRAINT chk_message_attachments_provider_format
        CHECK (provider ~ '^[a-z0-9_]+$'),

    CONSTRAINT chk_message_attachments_file_url_format
        CHECK (file_url IS NULL OR file_url ~* '^https?://'),

    CONSTRAINT chk_message_attachments_mime_type_format
        CHECK (mime_type IS NULL OR mime_type ~ '^[a-z0-9.+-]+/[a-z0-9.+-]+$'),

    CONSTRAINT chk_message_attachments_file_size
        CHECK (file_size_bytes IS NULL OR file_size_bytes > 0),

    CONSTRAINT chk_message_attachments_metadata_object
        CHECK (metadata IS NULL OR jsonb_typeof(metadata) = 'object')
);
...
CREATE TABLE IF NOT EXISTS appointments (
    ...
    CONSTRAINT uq_appointments_id_business_id
        UNIQUE (id, business_id),
    ...
    CONSTRAINT fk_appointments_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT fk_appointments_client_business
        FOREIGN KEY (client_id, business_id)
        REFERENCES clients(id, business_id),

    CONSTRAINT fk_appointments_worker_business
        FOREIGN KEY (worker_id, business_id)
        REFERENCES workers(id, business_id),

    CONSTRAINT fk_appointments_service_business
        FOREIGN KEY (service_id, business_id)
        REFERENCES services(id, business_id),

    CONSTRAINT fk_appointments_conversation_business
        FOREIGN KEY (conversation_id, business_id)
        REFERENCES conversations(id, business_id),

    CONSTRAINT fk_appointments_currency
        FOREIGN KEY (currency_code)
        REFERENCES currencies(code),
    ...
    CONSTRAINT chk_appointments_quoted_price
        CHECK (quoted_price IS NULL OR quoted_price >= 0),

    CONSTRAINT chk_appointments_expires_at
        CHECK (expires_at IS NULL OR expires_at > created_at),

    CONSTRAINT chk_appointments_user_creator_required
        CHECK (
...
CREATE UNIQUE INDEX IF NOT EXISTS appointments_idempotency_unique_idx
ON appointments (idempotency_key)
WHERE idempotency_key IS NOT NULL;

CREATE INDEX IF NOT EXISTS appointments_worker_time_idx
ON appointments (worker_id, start_at, end_at)
...
CREATE TABLE IF NOT EXISTS appointment_events (
    ...
    CONSTRAINT fk_appointment_events_appointment_business
        FOREIGN KEY (appointment_id, business_id)
        REFERENCES appointments(id, business_id),

    CONSTRAINT fk_appointment_events_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_appointment_events_created_by_user
        FOREIGN KEY (created_by_user_id)
        REFERENCES users(id),

    CONSTRAINT chk_appointment_events_old_values_object
        CHECK (old_values IS NULL OR jsonb_typeof(old_values) = 'object'),

    CONSTRAINT chk_appointment_events_new_values_object
        CHECK (new_values IS NULL OR jsonb_typeof(new_values) = 'object'),

    CONSTRAINT chk_appointment_events_user_creator_required
        CHECK (
            (created_by_type = 'user' AND created_by_user_id IS NOT NULL)
            OR
            (created_by_type IN ('ai', 'system'))
...
CREATE TABLE IF NOT EXISTS message_templates (
    ...
    CONSTRAINT fk_message_templates_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_message_templates_language
        FOREIGN KEY (language_code)
        REFERENCES languages(code),

    CONSTRAINT chk_message_templates_name_length
        CHECK (length(trim(name)) >= 2),

    CONSTRAINT chk_message_templates_body_length
        CHECK (length(trim(body)) >= 1),

    CONSTRAINT chk_message_templates_variables_schema_object
        CHECK (variables_schema IS NULL OR jsonb_typeof(variables_schema) = 'object')
);
...
CREATE TABLE IF NOT EXISTS business_ai_settings (
    ...
    CONSTRAINT fk_business_ai_settings_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_business_ai_settings_language
        FOREIGN KEY (default_language_code)
        REFERENCES languages(code),

    CONSTRAINT fk_business_ai_settings_welcome_template
        FOREIGN KEY (welcome_template_id)
        REFERENCES message_templates(id),

    CONSTRAINT fk_business_ai_settings_handoff_template
        FOREIGN KEY (handoff_template_id)
        REFERENCES message_templates(id),

    CONSTRAINT chk_business_ai_settings_ai_name_length
        CHECK (length(trim(ai_name)) >= 2),

    CONSTRAINT chk_business_ai_settings_max_tool_calls
...
CREATE TABLE IF NOT EXISTS business_whatsapp_accounts (
    ...
    CONSTRAINT fk_business_whatsapp_accounts_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_business_whatsapp_accounts_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT fk_business_whatsapp_accounts_phone_prefix
        FOREIGN KEY (phone_prefix_id)
        REFERENCES phone_prefixes(id),

    CONSTRAINT chk_business_whatsapp_accounts_phone_number_format
        CHECK (phone_number IS NULL OR phone_number ~ '^[0-9]{6,20}$'),

    CONSTRAINT chk_business_whatsapp_accounts_phone_e164_format
        CHECK (phone_e164 IS NULL OR phone_e164 ~ '^[+][0-9]{7,20}$')
);

CREATE UNIQUE INDEX IF NOT EXISTS business_whatsapp_accounts_global_per_business_idx
ON business_whatsapp_accounts (business_id)
WHERE branch_id IS NULL AND deleted_at IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS business_whatsapp_accounts_branch_unique_idx
ON business_whatsapp_accounts (business_id, branch_id)
...
CREATE TABLE IF NOT EXISTS calendar_connections (
    ...
    CONSTRAINT fk_calendar_connections_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_calendar_connections_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT fk_calendar_connections_worker_business
        FOREIGN KEY (worker_id, business_id)
        REFERENCES workers(id, business_id)
);
...
CREATE TABLE IF NOT EXISTS appointment_calendar_events (
    ...
    CONSTRAINT fk_appointment_calendar_events_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_appointment_calendar_events_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT fk_appointment_calendar_events_appointment_business
        FOREIGN KEY (appointment_id, business_id)
        REFERENCES appointments(id, business_id),

    CONSTRAINT fk_appointment_calendar_events_calendar_connection
        FOREIGN KEY (calendar_connection_id)
        REFERENCES calendar_connections(id),

    CONSTRAINT chk_appointment_calendar_events_metadata_object
        CHECK (metadata IS NULL OR jsonb_typeof(metadata) = 'object')
);
...
CREATE TABLE IF NOT EXISTS reminders (
    ...
    CONSTRAINT fk_reminders_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_reminders_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT fk_reminders_appointment_business
        FOREIGN KEY (appointment_id, business_id)
        REFERENCES appointments(id, business_id),

    CONSTRAINT fk_reminders_client_business
        FOREIGN KEY (client_id, business_id)
        REFERENCES clients(id, business_id),
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    CONSTRAINT fk_notifications_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),
...
CREATE TABLE IF NOT EXISTS plans (
    ...
    CONSTRAINT chk_plans_code_format
        CHECK (code ~ '^[a-z0-9_]+$'),

    CONSTRAINT chk_plans_name_length
        CHECK (length(trim(name)) >= 2),

    CONSTRAINT chk_plans_monthly_price
        CHECK (monthly_price >= 0),

    CONSTRAINT chk_plans_max_workers
        CHECK (max_workers IS NULL OR max_workers > 0),

    CONSTRAINT chk_plans_max_branches
        CHECK (max_branches IS NULL OR max_branches > 0),

    CONSTRAINT chk_plans_max_conversations
        CHECK (max_conversations_per_month IS NULL OR max_conversations_per_month >= 0),

    CONSTRAINT chk_plans_max_ai_messages
        CHECK (max_ai_messages_per_month IS NULL OR max_ai_messages_per_month >= 0),

    CONSTRAINT chk_plans_max_input_tokens
        CHECK (max_input_tokens_per_month IS NULL OR max_input_tokens_per_month >= 0),

    CONSTRAINT chk_plans_max_output_tokens
        CHECK (max_output_tokens_per_month IS NULL OR max_output_tokens_per_month >= 0)
);
...
CREATE TABLE IF NOT EXISTS business_subscriptions (
    ...
    CONSTRAINT fk_business_subscriptions_payment_provider
        FOREIGN KEY (payment_provider_id)
        REFERENCES payment_providers(id),

    CONSTRAINT chk_business_subscriptions_period
        CHECK (
...
CREATE TABLE IF NOT EXISTS usage_counters (
    ...
    CONSTRAINT fk_usage_counters_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),
    ...
    CONSTRAINT chk_usage_counters_conversations_count
        CHECK (conversations_count >= 0),

    CONSTRAINT chk_usage_counters_messages_count
        CHECK (messages_count >= 0),

    CONSTRAINT chk_usage_counters_ai_requests_count
        CHECK (ai_requests_count >= 0),

    CONSTRAINT chk_usage_counters_input_tokens
        CHECK (input_tokens >= 0),

    CONSTRAINT chk_usage_counters_output_tokens
        CHECK (output_tokens >= 0),

    CONSTRAINT chk_usage_counters_appointments_created_count
        CHECK (appointments_created_count >= 0),

    CONSTRAINT chk_usage_counters_whatsapp_messages_sent_count
        CHECK (whatsapp_messages_sent_count >= 0)
);

CREATE UNIQUE INDEX IF NOT EXISTS usage_counters_business_period_unique_idx
ON usage_counters (business_id, period_start, period_end);
...
CREATE TABLE IF NOT EXISTS audit_logs (
    ...
    CONSTRAINT fk_audit_logs_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),
    ...
    CONSTRAINT fk_audit_logs_conversation
        FOREIGN KEY (conversation_id)
        REFERENCES conversations(id),

    CONSTRAINT chk_audit_logs_action_format
        CHECK (action ~ '^[a-z0-9_]+[.][a-z0-9_]+$'),

    CONSTRAINT chk_audit_logs_old_values_object
        CHECK (old_values IS NULL OR jsonb_typeof(old_values) = 'object'),

    CONSTRAINT chk_audit_logs_new_values_object
        CHECK (new_values IS NULL OR jsonb_typeof(new_values) = 'object'),
...
CREATE TABLE IF NOT EXISTS business_members (
    ...
    CONSTRAINT fk_business_members_role
        FOREIGN KEY (role_id)
        REFERENCES roles(id)
);
...
CREATE TABLE IF NOT EXISTS business_invite_link_uses (
    ...
    CONSTRAINT fk_business_invite_link_uses_invite_link
        FOREIGN KEY (invite_link_id)
        REFERENCES business_invite_links(id),

    CONSTRAINT fk_business_invite_link_uses_user
        FOREIGN KEY (user_id)
        REFERENCES users(id)
);
...
CREATE TABLE IF NOT EXISTS worker_branches (
    ...
    CONSTRAINT fk_worker_branches_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_worker_branches_worker_business
        FOREIGN KEY (worker_id, business_id)
        REFERENCES workers(id, business_id),

    CONSTRAINT fk_worker_branches_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id)
);
...
CREATE TABLE IF NOT EXISTS worker_services (
    ...
    CONSTRAINT fk_worker_services_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_worker_services_worker_business
        FOREIGN KEY (worker_id, business_id)
        REFERENCES workers(id, business_id),

    CONSTRAINT fk_worker_services_service_business
        FOREIGN KEY (service_id, business_id)
        REFERENCES services(id, business_id),

    CONSTRAINT chk_worker_services_price_override
        CHECK (price_override IS NULL OR price_override >= 0),

    CONSTRAINT chk_worker_services_duration_override
...
CREATE TABLE IF NOT EXISTS branch_services (
    ...
    CONSTRAINT fk_branch_services_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),
...
CREATE TABLE IF NOT EXISTS client_branches (
    ...
    CONSTRAINT fk_client_branches_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),

    CONSTRAINT fk_client_branches_client_business
        FOREIGN KEY (client_id, business_id)
        REFERENCES clients(id, business_id),

    CONSTRAINT fk_client_branches_branch_business
        FOREIGN KEY (branch_id, business_id)
        REFERENCES branches(id, business_id),

    CONSTRAINT chk_client_branches_last_seen
...
CREATE TABLE IF NOT EXISTS notification_preferences (
    ...
    PRIMARY KEY (user_id, business_id, notification_type_id, channel),

    CONSTRAINT fk_notification_preferences_user
        FOREIGN KEY (user_id)
        REFERENCES users(id),

    CONSTRAINT fk_notification_preferences_business
        FOREIGN KEY (business_id)
        REFERENCES businesses(id),
...
ON businesses (country_id);

CREATE INDEX IF NOT EXISTS branches_business_idx
ON branches (business_id);

CREATE INDEX IF NOT EXISTS workers_business_idx
...
CREATE INDEX IF NOT EXISTS conversations_business_client_idx
ON conversations (business_id, client_id);
