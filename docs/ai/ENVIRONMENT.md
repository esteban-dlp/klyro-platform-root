...
`000-create-database` → `001-enums` → `002-tables` → `003-scheduling-functions` → seeds → ordered migrations through compose mount `060`. Order matters. Compose SQL runs automatically only for a fresh Postgres volume.
