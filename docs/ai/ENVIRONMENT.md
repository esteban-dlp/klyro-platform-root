...
| `DEEPINFRA_API_KEY` / `DEEPINFRA_BASE_URL` | backend (`.env.backend`) | when provider `deepinfra` is used | Existing DeepInfra credentials and OpenAI-compatible base URL; default `https://api.deepinfra.com/v1/openai`, whose SDK requests use `/chat/completions` |
